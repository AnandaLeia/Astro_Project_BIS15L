...
ui <- dashboardPage(
  ...
  dashboardSidebar(title="Eclipse Selections", 
                   ...
                   selectInput("y", " Solar or Lunar Eclipses:", choices=c("Solar", "Lunar"))),
...
server <- function(input, output, session) {
  output$map <- renderPlot({
    ...
    if (input$y == "Solar") {
      ggmap(map) + 
        geom_point(data=new_lunar2, aes(longitude_value, latitude_value), size=4) +
        labs(x= "Longitude", y= "Latitude", title="Lunar Eclipse Locations")
    } else {
      ggmap(map) + 
        geom_point(data=new_solar2, aes(longitude_value, latitude_value), size=4) + 
        labs(x="Longitude", y="Latitude", title="Solar Eclipse Locations")
