...
```{r}
solar$catalog_number <- as.numeric(solar$catalog_number)
```
```{r}
lunar$catalog_number <- as.numeric(lunar$catalog_number)
```
...
solar_filtered_dates3 <- solar %>% 
  filter(catalog_number<=10 & catalog_number>=1)
```
```{r}
ggmap(map_s)+
